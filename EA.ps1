...
function Load-ContentDataFromCache {
    param([string]$Path = 'C:\Program Files\EA\ContentData.cache.json')
    try {
...
$AppName = " Endpoint Advisor"
$MutexName = "Global\EA_SingleInstance_Mutex"
$LockFilePath = "$env:TEMP\EA_Instance.lock"
...
function Test-OtherInstanceRunning {
    ...
    try {
        ...
        foreach ($proc in $processes) {
            if ($proc.ProcessId -ne $CurrentPID -and $proc.CommandLine -like "*EA.ps1*") {
            ...
            if ($lockContent -match "PID:(\d+)") {
                ...
                if ($lockedProcess -and $lockedProcess.Id -ne $CurrentPID) {
                    try {
                        ...
                        if ($procInfo -and $procInfo.CommandLine -like "*EA.ps1*") {
...
function Write-Log {
    ...
    $logPath = if ($LogFilePath) { $LogFilePath } else { Join-Path $ScriptDir "EndpointAdvisor.log" }
        ...
        if (Test-Path $LogFilePath) {
            ...
            if ($fileInfo.Length -gt $maxSizeBytes) {
                ...
                try {
                    ...
                    $archiveFiles = Get-ChildItem -Path $LogDirectory -Filter "EndpointAdvisor.log.*.archive" | Sort-Object CreationTime
                    $maxArchives = 3
    ...
    try {
        ...
        if ($global:ADAccountStatus.Success -and $global:TrayIcon) {
            if ($global:ADAccountStatus.PasswordDaysLeft -ne $null -and $global:ADAccountStatus.PasswordDaysLeft -le 7) {
                $global:TrayIcon.Text = "EA - Password expires in $($global:ADAccountStatus.PasswordDaysLeft) days!"
            } elseif ($global:ADAccountStatus.AccountDaysLeft -ne $null -and $global:ADAccountStatus.AccountDaysLeft -le 7) {
                $global:TrayIcon.Text = "EA - Account expires in $($global:ADAccountStatus.AccountDaysLeft) days!"
            }
        }
...
function Start-PasswordChange {
    try {
        ...
        $message = @"
To change your COMPANY domain password:
...
Email: servicedesk@ll.somecompany.com
"@
...
Write-Log "---  Endpoint Advisor Script Started (Version $ScriptVersion) ---"
...
function Get-DefaultConfig {
    return @{
        ...
        YubiKeyAlertDays      = 14
        IconPaths             = @{
            Main    = Join-Path $ScriptDir "EA_LOGO.ico"
            Warning = Join-Path $ScriptDir "EA_LOGO_MSG.ico"
        }
...
function Load-Configuration {
    param([string]$Path = (Join-Path $ScriptDir "EndpointAdvisor.config.json"))
...
function Save-Configuration {
    param(
        [psobject]$Config,
        [string]$Path = (Join-Path $ScriptDir "EndpointAdvisor.config.json")
    )
    try {
...
# ============================================================
$LogFilePath = Join-Path $ScriptDir "EndpointAdvisor.log"
...
<Window
  ...
  <Grid Margin="5">
    ...
    <Border Grid.Row="0" Background="#2B579A" Padding="5" CornerRadius="3" Margin="0,0,0,5">
      <StackPanel Orientation="Horizontal" VerticalAlignment="Center" HorizontalAlignment="Center">
        ...
        <TextBlock Text=" Endpoint Advisor" FontSize="14" FontWeight="Bold" Foreground="White" VerticalAlignment="Center"/>
      </StackPanel>
    </Border>
  ...
  <StackPanel VerticalAlignment="Top">
    ...
    <Grid Grid.Row="2" Margin="0,5,0,0">
        ...
        <TextBlock x:Name="FooterText" Grid.Column="0" Text="(C) 2025 " FontSize="10" Foreground="Gray" HorizontalAlignment="Center" VerticalAlignment="Center"/>
...
try {
    ...
    $global:FooterText.Text = "(C) 2025  v$ScriptVersion"
...
function Get-DaysSinceLastDriverUpdate {
    $logFile = "C:\Windows\COMPANY\Logs\MS_Update.txt"
...
function Start-DriverUpdate {
    try {
        ...
        try {
            Start-ScheduledTask -TaskName "COMPANY_DriverUpdate" -ErrorAction Stop
            Write-Log "Scheduled task 'COMPANY_DriverUpdate' started successfully" -Level "INFO"
        } catch {
...
function Update-TrayIcon {
    ...
    $global:TrayIcon.Text = if ($hasAnyAlert) { "Endpoint Advisor v$ScriptVersion - Alerts Pending" } else { " Endpoint Advisor v$ScriptVersion" }
}
...
function Initialize-TrayIcon {
    ...
    try {
        ...
        $global:TrayIcon.Text = " Endpoint Advisor v$ScriptVersion"
...
try {
    ...
    Write-Log '---  Endpoint Advisor Script Exiting ---'
