###############################################################################
# SystemMonitor.ps1 - Tray Icon Always Visible Attempt
###############################################################################
...
function Write-Log {
    ...
    $logPath = if ($LogFilePath) { $LogFilePath } else { Join-Path $ScriptDir "SystemMonitor.log" }
...
$healthyIconPath = Join-Path $ScriptDir $config.IconPaths.Healthy
$warningIconPath = Join-Path $ScriptDir $config.IconPaths.Warning
$alertIconPath   = Join-Path $ScriptDir $config.IconPaths.Alert
$healthyIconUri  = "file:///" + ($healthyIconPath -replace '\\','/')
$alertIconUri    = "file:///" + ($alertIconPath -replace '\\','/')
  ...
  <Grid Margin="3">
    ...
    <ScrollViewer Grid.Row="1" VerticalScrollBarVisibility="Auto">
      <StackPanel VerticalAlignment="Top">
        ...
        <!-- Announcements with Icon in Header -->
        <Expander x:Name="AnnouncementsExpander" FontSize="12" Foreground="#000080" IsExpanded="False" Margin="0,2,0,2">
          <Expander.Header>
            <StackPanel Orientation="Horizontal">
              <TextBlock Text="Announcements" VerticalAlignment="Center"/>
              <Image x:Name="AnnouncementsAlertIcon" Source="$alertIconUri" Width="16" Height="16" Margin="4,0,0,0" Visibility="Hidden"/>
            </StackPanel>
          </Expander.Header>
...
function Update-SystemInfo {
    try {
        ...
        if ($global:yubiKeyJob -and $global:yubiKeyJob.State -eq "Completed") {
            ...
            $YubiKeyCertExpiryText.Text = $yubiKeyResultString
        ...
        elseif (-not $global:yubiKeyJob) {
            ...
            if ($checkYubiKey) {
                ...
                $YubiKeyCertExpiryText.Text = $config.YubiKeyLastCheck.Result
            }
        }
    }
    catch {
...
function Update-Announcements {
    try {
        ...
        if ($changes.Count -gt 0 -and -not $AnnouncementsExpander.IsExpanded) {
            ...
            Write-Log "Announcements alert icon set to visible" -Level "INFO"
...
function Update-PatchingUpdates {
    try {
        ...
        Write-Log "Patching status updated: $($PatchingUpdatesText.Text)" -Level "INFO"
    }
    catch {
...
function Update-TrayIcon {
    try {
        ...
        if ($antivirusStatus -and $bitlockerStatus -and $yubikeyStatus -and $code42Status -and $fipsStatus -and $bigfixStatus) {
            ...
            Write-Log "Tray icon set to healthy.ico" -Level "INFO"
...
$AnnouncementsExpander.Add_Expanded({
    if ($global:announcementAlertActive) {
        $AnnouncementsAlertIcon.Visibility = "Hidden"
        $global:announcementAlertActive = $false
        Write-Log "Announcements alert icon hidden on expand" -Level "INFO"
    }
})
...
$TrayIcon.Visible = $true  # Explicitly set to true on creation
Write-Log "Tray icon initialized with healthy.ico" -Level "INFO"
Write-Log "Note: To ensure the System Monitor tray icon is always visible, right-click the taskbar, select 'Taskbar settings', scroll to 'Notification area', click 'Select which icons appear on the taskbar', and set 'System Monitor' to 'On'." -Level "INFO"
...
$MenuItemExit.add_Click({
    try {
        ...
        $TrayIcon.Visible = $false  # Ensure cleanup before dispose
        $TrayIcon.Dispose()
